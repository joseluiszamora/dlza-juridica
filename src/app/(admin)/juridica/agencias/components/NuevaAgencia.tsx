...
const NuevaAgencia: React.FC<Props> = ({ onSave }) => {
  ...
  const [tipoGarantia, setTipoGarantia] = useState("efectivo");
  const [contratoInicio, setContratoInicio] = useState<Date | null>(new Date());
  const [contratoFin, setContratoFin] = useState<Date | null>(new Date());
  const [vigenciaLicencia, setVigenciaLicencia] = useState<Date | null>(null);
  const [licenciaFuncionamiento, setLicenciaFuncionamiento] = useState(false);
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    if (!agenteId || !ciudadId) {
      // toast({
      //   title: "Error",
      //   description: "Debe seleccionar un agente y una ciudad",
      //   variant: "error"
      // });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAgenteId(null);
        setCiudadId(null);
        setTipoGarantia("efectivo");
        setLatitud(null);
        setLongitud(null);
        setContratoInicio(new Date());
        setContratoFin(new Date());
        setVigenciaLicencia(null);
        setLicenciaFuncionamiento(false);
        onSave();
      } else {
        ...
        // toast({
        //   title: "Error",
        //   description: errorData.error || "Error al crear agencia",
        //   variant: "error"
        // });
      }
    } catch (error) {
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-1">
              ...
              <div className="mt-5">
                <div className="flex items-center mt-3 mb-2">
                  ...
                  />
                  <label htmlFor="licenciaFuncionamiento" className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">
